  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!pinIsSet || !pinHashValue) {
        // First-time admin — need to set a PIN
        if (!newPin || !confirmPin) {
          setPendingAdminId(adminUser.id);
          setIsFirstTime(true);
          setError('Welcome! This is your first login. Please create a 6-digit PIN below.');
          setLoading(false);
          return;
        }
      ...
      const pinToVerify = isFirstTime ? newPin : adminPin;

      if (!pinToVerify || pinToVerify.length !== 6 || !/^\d{6}$/.test(pinToVerify)) {
        setError('Admin PIN must be exactly 6 digits.');
        setLoading(false);
        return;
      }

      const pinHash = await hashPin(pinToVerify);
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* PIN — shown for returning admins or after first-time prompt */}
            {!isFirstTime ? (
              <div>
                <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--foreground-muted)' }}>
                  6-Digit Admin PIN
              ...
              <>
                ...
                >
                  ✦ First login detected. Create your 6-digit PIN below.
                </div>
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: 'var(--foreground-muted)' }}>
                    Create 6-Digit PIN
            ...
            {error && (
              <p className="text-xs text-center" style={{ color: isFirstTime && error.includes('first login') ? '#f59e0b' : '#f87171' }}>
                {error}
              </p>
            )}
            ...
            <button
              ...
              suppressHydrationWarning
            >
              {loading ? 'Signing in...' : isFirstTime ? 'Set PIN & Sign In ✦' : 'Sign In to Admin ✦'}
            </button>
